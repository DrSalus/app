...
export default function Home() {
  ...
  useEffect(() => {
    fetch("/comuni.json")
      ...
      .then((r) => {
        ...
        setCitiesOptions(
          cities.map((c) => ({ value: c.nome, label: `${c.nome}, ${c.sigla}` }))
        );
      });
  }, []);
  ...
  return (
    <div className="h-screen flex flex-col">
      <div className="h-1/2 relative bg-gradient-to-b from-sky-800 to-sky-700 flex flex-col items-center justify-center">
        <img
          src="/hero.jpg"
        ...
        <div className="z-10  relative flex items-stretch gap-x-2 h-14 shadow-lg  rounded w-full max-w-3xl">
      ...
      <Show if={disabled}>
        <div className="h-1/2 flex flex-col items-center px-4 py-2 justify-center">
          <div className="text-3xl flex items-center font-bold text-primary mt-4">
      ...
      <Show unless={disabled}>
        <div className="h-1/2 flex flex-nowrap items-stretch overflow-scroll gap-x-4 p-4 max-w-full">
          {agendas.map((agenda) => (
            <div
              key={agenda.id}
